...
SOURCE_URLS = {
    "Heimspiel": "https://tickets.vfb.de/shop?language=1&shopid=103&nextstate=2&lpShortcutId=5",
    "Auswärtsspiel": "https://tickets.vfb.de/shop?language=1&shopid=103&nextstate=2&lpShortcutId=4",
}
...
def vevent(kind: str, title: str, start: datetime, end: datetime,
           description: str, source_url: str, location: str = "", alarm_minutes: int | None = None) -> list[str]:
...
def get_page_text(source_url: str) -> str:
...
def parse_events(text: str, source_label: str, source_url: str) -> list[list[str]]:
    ...
    for match in DATE_RE.finditer(text):
        ...
        prefix = "VfB-Heimspiel" if source_label == "Heimspiel" else "VfB-Auswärtsspiel"
        title = f"{prefix}: VfB Stuttgart – {opponent}"
        ...
        events[key] = vevent(
            ...
            f"{source_label} aus dem offiziellen VfB-Ticketshop. Angaben können sich ändern.\n" + context,
            source_url,
            "MHP Arena Stuttgart" if source_label == "Heimspiel" else "",
        )
        ...
        for sale in SALE_RE.finditer(block):
            ...
            events[sale_key] = vevent(
                ...
                f"Verkaufsstart für ein {source_label.lower()} laut offiziellem VfB-Ticketshop. Frühzeitig einloggen.",
                source_url,
                alarm_minutes=30,
            )
...
def main() -> None:
    ...
    for source_label, source_url in SOURCE_URLS.items():
        try:
            text = get_page_text(source_url)
            blocks = parse_events(text, source_label, source_url)
            if not blocks:
                raise RuntimeError("Keine Termine auf der gerenderten Shopseite erkannt.")
            all_blocks.extend(blocks)
            print(f"{source_label}: {len(blocks)} Kalendereinträge erkannt.")
        except Exception as exc:
            errors.append(f"{source_label}: {exc}")
            print(f"WARNUNG bei {source_label}: {exc}")

    if all_blocks:
        write_calendar(all_blocks)
        print(f"Insgesamt {len(all_blocks)} Kalendereinträge geschrieben.")
        if errors:
            print("Mindestens eine Quelle konnte diesmal nicht gelesen werden.")
        return

    # Nur wenn beide Quellen fehlschlagen, bleibt die letzte funktionierende Datei erhalten.
    if OUT.exists() and OUT.stat().st_size > 100:
        print("Beide Quellen konnten nicht gelesen werden.")
        print("Vorhandene Kalenderdatei bleibt unverändert bestehen.")
        return

    raise RuntimeError("Keine Termine aus Heim- oder Auswärtsspielquelle erkannt.")
